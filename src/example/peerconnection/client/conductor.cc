...
bool Conductor::InitializePeerConnection() {
  ...
  peer_connection_factory_ = webrtc::CreatePeerConnectionFactory(
      ...
      new webrtc::WinObsVideoEncoderFactory(),
      nullptr);
...
void Conductor::OnSuccess(webrtc::SessionDescriptionInterface* desc) {
  webrtc::JsepSessionDescription *jsep = (webrtc::JsepSessionDescription*)desc;
  cricket::SessionDescription *sDsc = jsep->description();
  cricket::ContentInfos &contents = sDsc->contents();
  for (cricket::ContentInfo content : contents) {
    if (content.name == "video") {
      cricket::MediaContentDescriptionImpl<cricket::VideoCodec> *description = 
          (cricket::MediaContentDescriptionImpl<cricket::VideoCodec>*)content.description;
      ...
      for (auto iter = codecs.begin(); iter != codecs.end();) {
        ...
        //Remove VP8,VP9,RTX.
        if (codec.name == "VP8" || codec.name == "VP9" || codec.name == "rtx") {
          iter = codecs.erase(iter);
        } else {
          //Keep only profile 42e01f for WinObsVideoEncoderFactory fix it to 42e01f,
          //of course you can set to another value, but make sure they're the same.
          if (codec.name == "H264") {
            if (codec.params["profile-level-id"] != "42e01f") {
              iter = codecs.erase(iter);
            } else {
              iter++;
            }
  ...
  OutputDebugStringA("OnSuccess:\n");
  OutputDebugStringA(sdp.c_str());
  OutputDebugStringA("\n");
